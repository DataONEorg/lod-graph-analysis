...
This document presents code to build a network of datasets from a DataONE archive.  It takes as input an `.Rdata` file that was prepared in a separate document "events_munging.R"  See the section "Import the data file" below for details.
...
The entire archive captured in `events.csv` is too large to build with the code in this document.  So we subset the archive by node id (where "node" here is DataONE storage node) and build a network from subsets of the entire archive.
...
head(datasets)
rm(events, node_freq, smaller_nodes)
```
...
rm(degree_df, eigen_clus, eigen_df, walktrap_clus, walktrap_df, unique_pids)
